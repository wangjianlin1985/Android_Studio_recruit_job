...
public class DeliveryQiyeEditActivity extends Activity {
	// 声明确定添加按钮
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// 设置当前Activity界面布局
		setContentView(R.layout.delivery_qiye_edit); 
		ImageView search = (ImageView) this.findViewById(R.id.search);
		search.setVisibility(View.GONE);
		TextView title = (TextView) this.findViewById(R.id.title);
		title.setText("处理职位投递信息");
			...
			public void onClick(View v) {
				try {
					...
					if(ET_deliveryTime.getText().toString().equals("")) {
						Toast.makeText(DeliveryQiyeEditActivity.this, "投递时间输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_deliveryDemo.getText().toString().equals("")) {
						Toast.makeText(DeliveryQiyeEditActivity.this, "企业回复输入不能为空!", Toast.LENGTH_LONG).show();
					...
					/*调用业务逻辑层上传职位投递信息*/
					DeliveryQiyeEditActivity.this.setTitle("正在更新职位投递信息，稍等...");
	...
	private void initViewData() {
		...
		this.spinner_jobObj.setEnabled(false);
		...
		this.spinner_userObj.setEnabled(false);
