...
public class JobDetailActivity extends Activity {
	// 声明返回按钮
	private Button btnDelivery,btnReturn;
	// 声明职位id控件
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		initViewData();
		final Declare declare = (Declare) JobDetailActivity.this.getApplication();
		if(declare.getIdentify().equals("qiye")) btnDelivery.setVisibility(View.GONE);
		
		btnDelivery.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				Delivery delivery = new Delivery();
				delivery.setJobObj(job.getJobId());
				
				delivery.setUserObj(declare.getUserName());
				delivery.setDeliveryTime("");
				delivery.setStateObj(1);
				delivery.setDeliveryDemo("");
				/*调用业务逻辑层上传职位投递信息*/
